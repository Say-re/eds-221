...
```{r}
mack_creek_vertebrates <- read_csv(here::here("activity", "AS00601.csv"))
```
...
mack_creek_lengths <- mack_creek_vertebrates %>% 
  ...
  ungroup()
view(mack_creek_lengths)
```

### reprex 1: Solution
```{r}
# There is only 1 issue present in the first example. The across function being called when trying to mutate the data frame is not being supplied 2 arguments.
# Instead, the where() function is being supplied 2 arguments when it only needs one
mack_reprex_example <- mack_creek_vertebrates |> # Reference read in file
  dplyr::mutate(across(where(is.character), tolower)) # Ensure 'tolower' is the second argument for the across() function.

View(mack_reprex_example)
```
