...
knb_files <- list.files(here::here("activity", "knb-lter-cap_256_9"), pattern = "*.csv")
knb_files
...
for (i in seq_along(knb_files)) {
  assign(x = knb_files[i] |> 
          str_remove("52_pp52_") |> 
          str_remove(".csv"),
         value = read_csv(here::here("activity", "knb-lter-cap_256_9", 
                       knb_files[i])))
}

# Verify each file has been read in under desired alias
# view(birds_1)
# view (taxalist_1)
# view(surveys_1)
# view(sites_1)
...
```{r}
ggsave("bird_counts.jpg", height = 6, width = 12)
```
